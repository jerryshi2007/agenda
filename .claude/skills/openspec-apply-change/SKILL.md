...
**与 `dev-sdd` 的关系**：本 skill 是轻量级实现路径（直接实现，适合简单变更，task 数 ≤2）。复杂变更（多 task / 大 task）应使用 `dev-sdd` skill（子代理驱动开发）。选择规则见 `dev-sdd` skill。
...
All tasks complete! 接下来进入收尾链（遵循 `dev-sdd` skill 的收尾链定义）：
