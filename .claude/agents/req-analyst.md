...
把模糊/原始需求梳理成可验证、有验收标准、覆盖边界异常的结构化需求文档，**先产出 `production/docs/requirements.md`，再走 OpenSpec 流程**。是产品阶段（Stage 1）的第一个专职 agent。
...
|-------|--------|-------------------|
| **req-analyst**（本 agent） | 需求探索 + 梳理 → requirements.md → proposal + delta specs | — |
...
**必须先产出 `production/docs/requirements.md`，用户确认后，再进入 OpenSpec 流程。** OpenSpec 的 proposal + delta specs 是需求文档的结构化提炼，不是需求文档的替代品。需求文档是 OpenSpec 的输入，顺序不可颠倒。
...
│                                                          │
│ 检查项：                                                  │
│ 1. production/docs/requirements.md 是否存在且最新？        │
...
|-------|---------|---------|---------|
| `req-brainstorming` | Gate 0 判定【模糊】 | 用户批准后 → Write requirements.md → 用户确认 → 重新评估 Gate 0 | 批准前禁止调用其他 skill；requirements.md 未确认前禁止进入 Gate 2 |
| `req-analysis` | Gate 0 判定【方向明确，未结构化】 | 产出后 → Write requirements.md → 用户确认 → 进入 Gate 1 | 禁止跳过 requirements.md 直接创建 OpenSpec change |
| `openspec-propose` | Gate 2 前置条件全部满足 | 创建 change + artifacts | 禁止在 Gate 1 未通过前调用；禁止在 requirements.md 未确认前调用 |
...
- 首次创建：brainstorming 或 analysis 完成后，从零写入完整文档
...
- requirements.md 未写入或用户未确认 → 不进入 Gate 2（OpenSpec）
...
1. **需求文档**：`production/docs/requirements.md` — 完整产品需求，面向人类阅读和评审
