...
```
app/
├── miniapp/         # 本目录：正式应用项目（源码，微信开发者工具打开此目录）
└── miniapp-test/    # 测试项目（Jest），引用 miniapp/ 源码
```

## 构建/调试命令

- 使用**微信开发者工具**打开 `app/miniapp/` 目录
- 微信开发者工具编译预览 → 生成小程序码在手机预览
- 结构/契约校验：`cd app/miniapp && npm run build`（语法/JSON/页面四件套）、`npm run lint`（契约合规与安全底线）
- 单元测试：`cd app/miniapp-test && npm test`（Jest）
- 无需 `npm install`（本项目使用原生框架，运行时不依赖 npm 包；`package.json` 仅承载 build/lint 两个 node 内置脚本）
