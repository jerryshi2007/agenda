...
| 现状符号 | 位置 | 本次处置 |
|---------|------|---------|
| `Schedule.AssignedChildId`（Guid，存 **User.Id**，非 FamilyMember.Id） | `api/Domain/Entities/Schedule.cs:15` | **扩展**：语义泛化为「成员 User.Id」，列零迁移，C# 属性改名 `AssignedMemberId` + `[Column("AssignedChildId")]` |
...
| `AppDbContext` / `Configurations` | `api/Infrastructure/Data/` | **复用**（无新实体/新表，无 Migration） |
...
**Goals**：
- 关联对象泛化到「成员」，家长孩子共用同一 `Schedule` 模型，零数据迁移、向后兼容
...
### Decision 1: 身份语义 —— `AssignedChildId` 存 `User.Id`，泛化为成员 User.Id（零迁移）

**Context**：现状 `AssignedChildId` 存的是 `User.Id`（非 `FamilyMember.Id`），家长与孩子共用 `FamilyMember` 表靠 `Role` 区分。需在「零迁移」硬约束下把关联对象从孩子扩展为成员。

**Decision**：
- **数据层零迁移**：`AssignedChildId` 列保持 Guid 不变，语义从「孩子的 User.Id」扩展为「成员的 User.Id」（家长或孩子）。不新增 `FamilyMember.Id` 外键、不改列类型、不回填数据。
- **代码层改名**：C# 实体属性 `AssignedChildId` → `AssignedMemberId`，用 `[Column("AssignedChildId")]` 映射保持 DB 列名；DTO 内部属性改用新名（`MemberIds`/`AssignedMemberId`/`MemberId`）。**API 边界的新旧并存由 Decision 6 兼容层负责**，内部命名不受兼容层影响。
- **成员角色反查**：`AssignedMemberId(User.Id) + FamilyId → FamilyMembers(Role)`。双文案、打卡权限、streak 排除均需此反查。

**Alternatives Considered**：
- ❌ 引入 `FamilyMember.Id` 外键（`AssignedMemberId` 指向 memberId）：语义更严谨，但需迁移列语义 + 回填存量 `User.Id → FamilyMember.Id`，违反零迁移硬约束，改动面大。
- ❌ 保留 `AssignedChildId` 字段名不改：最小改动但命名误导（存家长 User.Id 却叫 ChildId），违反「命名表意图」。

**Consequences**：
- ✅ 零迁移、向后兼容，存量孩子日程行完全不变
- ✅ 成员角色反查复用 `FamilyMembers` 已有索引（UserId+FamilyId）
- ⚠️ DB 列名仍是 `AssignedChildId`（内部名），需在实体注释 + 契约中显式说明「该列存成员 User.Id」
...
- **数据库**：单 PostgreSQL 库，**无 Migration**（零迁移）；契约域新增 `openspec/contracts/schedule/`。
...
│   (列名 AssignedChildId) │   N 成员 = N 行，GroupKey 关联同一「日程」
│ FamilyId · GroupKey      │
...
2. **后端实体/DTO 改名 + 兼容层**：`Schedule.AssignedMemberId` + DTO 新字段 + API 边界新旧并存（`childIds`/`assignedChildId` deprecated 别名 + 归一化/双输出）
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| `AssignedChildId` 列名与「成员」语义不符（DB 层） | 代码可读性下降 | 实体注释 + 契约显式说明；C# 属性已改名 `AssignedMemberId`，仅 DB 列名保留（零迁移成本） |
